...
test('DB file exists', os.path.exists(config.db_path))
...
# ============================================
test('1.1 Users exist', len(users) > 0, f'{len(users)} users')
test('1.2 Admin role=admin', admin_user['role'] == 'admin')
test('1.3 Admin has machine_id', bool(admin_user.get('machine_id')))
test('1.4 Non-admin (requester) exists', any(u['role'] == 'requester' for u in users))
...
test('3.0 SCs queryable', isinstance(all_scs, list), f'{len(all_scs)} SCs')
...
try:
    ...
    test('3.1 Create SC draft', bool(draft.get('sc_id')), f'ID={draft.get("sc_id")}')
    sc_id = draft['sc_id']

    expected = f'SC-{requester_user["machine_id"]}-{today}'
    test('3.2 SC ID format', sc_id.startswith(expected), sc_id)
    ...
    test('3.3 SC detail accessible', bool(detail.get('sc')), 'OK')
    test('3.4 SC status = pending', detail['sc']['status']=='pending', f'status={detail["sc"]["status"]}')
    ...
    test('3.5 Approve: pending->approved', d3['sc']['status']=='approved')

    sc_service.close_sc(config, admin_user, sc_id)
    d4 = sc_service.get_sc_detail(config, admin_user, sc_id)
    test('3.6 Close: approved->closed', d4['sc']['status']=='closed')
    ...
    test('3.7 Deny: pending->denied', dd['sc']['status']=='denied')

    upd = sc_service.update_sc(config, admin_user, sc_id2, {'description':'Updated'})
    test('3.8 Edit SC', upd is not None)
    ...
    test('3.9 SC#3 approved for PO/GR tests', True, f'ID={sc_id3}')
...
test('4.0 POs queryable', isinstance(all_pos, list), f'{len(all_pos)} POs')
...
if test_sc_id and len(vendors) > 0:
    try:
        ...
        test('4.1 Create PO', bool(po.get('po_id')), f'ID={po.get("po_id")}')
        po_id = po['po_id']
        test('4.2 PO ID format', bool(re.match(r'PO-\w+-\d{8}-\d{3}$', po_id)), po_id)

        detail = sc_service.get_sc_detail(config, admin_user, test_sc_id)
        test('4.3 PO in SC detail', len(detail.get('pos',[]))>0, f'{len(detail.get("pos",[]))} POs')
        ...
        test('4.4 Approve PO', pf and pf['status']=='po_approved')

        # Edit PO
        po_service.update_po(config, admin_user, po_id, {'contract_no':'CT-MODIFIED'})
        test('4.5 Edit PO', True)
...
test('5.0 GRs queryable', isinstance(all_grs, list), f'{len(all_grs)} GRs')
...
if test_po_id:
    try:
        ...
        test('5.1 Create GR', bool(gr.get('gr_id')), f'ID={gr.get("gr_id")}')
        gr_id = gr['gr_id']
        test('5.2 GR ID format', bool(re.match(r'GR-\w+-\d{8}-\d{3}$', gr_id)), gr_id)

        gr_service.update_gr(config, admin_user, gr_id, {'estimated_amount':1800.00})
        test('5.3 Edit GR', True)
        ...
        test('5.4 Approve GR', gf and gf['status']=='approved', f'con={gf.get("con_value") if gf else "N/A"}')
        ...
        test('5.5 Cancel GR', gf2 and gf2['status']=='cancelled')
    ...
    if test_po_id:
        try:
            ...
            test('4.6 Finish PO', pf2 and pf2['status']=='finished')
        except Exception as e:
...
try:
    ...
    test('6.1 Create vendor', bool(v.get('vendor_id')), f'ID={v.get("vendor_id")}')
    v_id = v['vendor_id']

    vendor_service.update_vendor(config, admin_user, v_id, {'phone':'13900139000'})
    test('6.2 Edit vendor', True)
    ...
    test('6.3 Disable vendor', vf and vf.get('status')=='disabled')
except Exception as e:
...
test('7.1 Audit logs', len(log_rows) > 0, f'{len(log_rows)} entries')
...
test('7.2 Expected actions logged', len(found) >= 10, f'{len(found)}/{len(expected)}: {found[:8]}')
...
try:
    ...
    test('8.1 Notification queue', True, f'{len(q)} entries')
except Exception as e:
    test('8.1 Notification queue', False, str(e))
...
try:
    ...
    test('9.3 Disable user', ud is None, 'removed from active list (list_active_users filters)')
    ...
    test('9.4 Enable user', ue is not None, 'back in active list')

    defaults = notification_service.get_notification_defaults(config)
    test('9.5 Notification defaults', True, f'keys: {list(defaults.keys())[:5]}')
except Exception as e:
...
for item in ['Workbench','SC','PO','GR','Vendor','Email','Logs','System']:
    test(f'11.1 Sidebar: {item}', item in sidebar)

with open('frontend/src/components/layout/AppHeader.vue', 'r', encoding='utf-8') as f:
    header = f.read()
test('11.2 Breadcrumb data-driven', 'breadcrumbs = computed(' in header)

# ============================================
hr('Section 12: Auto-Generated IDs')
# ============================================
if test_sc_id:
    test('12.1 SC ID SC-{M}-{D}-{SEQ}', bool(re.match(r'SC-\w+-\d{8}-\d{3}$', test_sc_id)), test_sc_id)
if test_po_id:
    test('12.2 PO ID PO-{M}-{D}-{SEQ}', bool(re.match(r'PO-\w+-\d{8}-\d{3}$', test_po_id)), test_po_id)
if test_gr_id:
    test('12.3 GR ID GR-{M}-{D}-{SEQ}', bool(re.match(r'GR-\w+-\d{8}-\d{3}$', test_gr_id)), test_gr_id)
if test_sc_id and test_sc_id2:
    s1, s2 = int(test_sc_id[-3:]), int(test_sc_id2[-3:])
    test('12.4 SC sequence increments', s1 > s2, f'{s2} -> {s1} (SC#2 before SC#3)')
