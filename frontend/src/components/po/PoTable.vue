<template>
  ...
  >
    ...
    <el-table-column prop="status" :label="$t('po.status')" width="100" sortable="custom">
      <template #default="{ row }"><StatusBadge :status="row.status" /></template>
    </el-table-column>
    <el-table-column prop="po_no" :label="$t('po.poNo')" width="130" sortable="custom">
    ...
    <el-table-column v-if="!hideScInfo" prop="sc_id" :label="$t('filter.scId')" width="130" sortable="custom">
      <template #default="{ row }">
    ...
    <el-table-column v-if="!hideScInfo" prop="requester_name" :label="$t('filter.requesterName')" width="130" show-overflow-tooltip sortable="custom" />
    <el-table-column prop="vendor_name" :label="$t('po.vendor')" width="160" show-overflow-tooltip sortable="custom" />
    ...
    <el-table-column prop="contract_type" :label="$t('po.contractType')" width="100" sortable="custom" />
    <el-table-column prop="contract_no" :label="$t('po.contractNo')" width="130" sortable="custom">
      <template #default="{ row }">{{ row.contract_no || '-' }}</template>
    </el-table-column>
    <el-table-column prop="payment_frequency" :label="$t('po.paymentFrequency')" width="110" sortable="custom">
      <template #default="{ row }">{{ row.payment_frequency || '-' }}</template>
    </el-table-column>
    <el-table-column prop="contract_pos" :label="$t('po.contractPos')" width="100" sortable="custom">
      <template #default="{ row }">{{ row.contract_pos || '-' }}</template>
    </el-table-column>
    <el-table-column prop="cost_center" :label="$t('po.costCenter')" width="110" sortable="custom" />
    <el-table-column prop="purchaser" :label="$t('po.purchaser')" width="110" sortable="custom">
      <template #default="{ row }">{{ row.purchaser || '-' }}</template>
    </el-table-column>
    <el-table-column prop="po_amount" :label="$t('po.poAmount')" width="120" sortable="custom">
    ...
    <el-table-column prop="po_pending_total_incl_tax" :label="$t('po.pendingInclTax')" width="140">
      <template #default="{ row }"><AmountDisplay :value="row.po_pending_total_incl_tax" /></template>
    </el-table-column>
    <el-table-column prop="consumed_amount" :label="$t('po.consumedAmount')" width="130">
      <template #default="{ row }"><AmountDisplay :value="row.consumed_amount" /></template>
    </el-table-column>
    <el-table-column prop="created_at" :label="$t('timestampLabel.created')" width="120" sortable="custom">
      <template #default="{ row }">{{ formatDate(row.created_at) }}</template>
    </el-table-column>
    <el-table-column prop="contract_from" :label="$t('po.startDate')" width="120" sortable="custom">
      <template #default="{ row }">{{ formatDate(row.contract_from) }}</template>
    </el-table-column>
    <el-table-column prop="contract_to" :label="$t('po.contractEndDate')" width="120" sortable="custom">
      <template #default="{ row }">{{ formatDate(row.contract_to) }}</template>
    </el-table-column>
    <el-table-column prop="active_date" :label="$t('po.activeDate')" width="120" sortable="custom">
    ...
    <el-table-column prop="updated_at" :label="$t('timestampLabel.updated')" width="120" sortable="custom">
