<template>
  ...
  >
    ...
    <el-table-column prop="status" :label="$t('po.status')" width="90" sortable>
      <template #default="{ row }"><StatusBadge :status="row.status" /></template>
    </el-table-column>
    <el-table-column prop="po_no" :label="$t('po.poNo')" width="115" sortable>
    ...
    <el-table-column v-if="!hideScInfo" prop="sc_no" :label="$t('filter.scNo')" width="115" sortable show-overflow-tooltip>
      <template #default="{ row }">{{ row.sc_no || '-' }}</template>
    </el-table-column>
    <el-table-column v-if="!hideScInfo" prop="sc_id" :label="$t('filter.scId')" width="115" sortable>
      <template #default="{ row }">
    ...
    <el-table-column v-if="!hideScInfo" prop="requester_name" :label="$t('filter.requesterName')" width="100" show-overflow-tooltip sortable />
    <el-table-column prop="vendor_name" :label="$t('po.vendor')" width="140" show-overflow-tooltip sortable>
      <template #default="{ row }">
    ...
    <el-table-column prop="contract_type" :label="$t('po.contractType')" width="90" sortable />
    <el-table-column prop="contract_no" :label="$t('po.contractNo')" width="115" sortable show-overflow-tooltip>
      <template #default="{ row }">{{ row.contract_no || '-' }}</template>
    </el-table-column>
    <el-table-column prop="contract_from" :label="$t('po.startDate')" width="100" sortable>
      <template #default="{ row }">{{ formatDate(row.contract_from) }}</template>
    </el-table-column>
    <el-table-column prop="contract_to" :label="$t('po.contractEndDate')" width="100" sortable>
      <template #default="{ row }">{{ formatDate(row.contract_to) }}</template>
    </el-table-column>
    <el-table-column prop="active_date" :label="$t('po.activeDate')" width="100" sortable>
      <template #default="{ row }">{{ formatDate(row.active_date) }}</template>
    </el-table-column>
    <el-table-column prop="payment_frequency" :label="$t('po.paymentFrequency')" width="95" sortable>
      <template #default="{ row }">{{ row.payment_frequency || '-' }}</template>
    </el-table-column>
    <el-table-column prop="contract_pos" :label="$t('po.contractPos')" width="90" sortable>
      <template #default="{ row }">{{ row.contract_pos || '-' }}</template>
    </el-table-column>
    <el-table-column prop="cost_center" :label="$t('po.costCenter')" width="95" sortable />
    <el-table-column prop="purchaser" :label="$t('po.purchaser')" width="95" sortable show-overflow-tooltip>
      <template #default="{ row }">{{ row.purchaser || '-' }}</template>
    </el-table-column>
    <el-table-column prop="po_amount" :label="$t('po.poAmount')" width="110" sortable>
      <template #default="{ row }"><AmountDisplay :value="row.po_amount" /></template>
    </el-table-column>
    <el-table-column prop="open_po_amount" :label="$t('po.openCon')" width="110" sortable>
      <template #default="{ row }"><AmountDisplay :value="row.open_po_amount" /></template>
    </el-table-column>
    <el-table-column prop="po_pending_total_incl_tax" :label="$t('po.pendingInclTax')" width="120" sortable>
      <template #default="{ row }"><AmountDisplay :value="row.po_pending_total_incl_tax" /></template>
    </el-table-column>
    <el-table-column prop="consumed_amount" :label="$t('po.consumedAmount')" width="115" sortable>
